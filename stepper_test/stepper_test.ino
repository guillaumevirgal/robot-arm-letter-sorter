...
// ── Pin definitions ────────────────────────────────────────────────────────────
#define BASE_STEP       2           // step pulse pin for base stepper
#define BASE_DIR        4           // direction pin for base stepper
#define SHOULDER_STEP   3           // step pulse pin for shoulder stepper
#define SHOULDER_DIR    5           // direction pin for shoulder stepper
#define PIN_ELBOW       6           // PWM pin for elbow servo
#define PIN_GRIPPER     10          // PWM pin for gripper servo
...
#define STEP_DELAY_US   300         // microseconds between step pulses — controls speed
...
void setup() {
  ...
  servo_elbow.write(90);              // start elbow at neutral position
  servo_gripper.write(90);            // start gripper at neutral position
  ...
  // ── 1. Base stepper ─────────────────────────────────────────────────────────
  Serial.println("Base: rotating 30 deg");
  rotateStepper(BASE_STEP, BASE_DIR, TEST_ANGLE);   // rotate base 30°
  delay(DELAY_STEP_MS);                             // 3 second pause


  // ── 2. Shoulder stepper ─────────────────────────────────────────────────────
  Serial.println("Shoulder: rotating 30 deg");
  rotateStepper(SHOULDER_STEP, SHOULDER_DIR, TEST_ANGLE);   // rotate shoulder 30°
  delay(DELAY_STEP_MS);                                     // 3 second pause
  ...
  servo_gripper.write(90 + TEST_ANGLE); // move from neutral (90°) by +30°
  delay(DELAY_STEP_MS);                 // 3 second pause
